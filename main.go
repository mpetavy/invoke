...
func init() {
	common.Init("1.0.3", "2017", "invokes a command to measure times", "mpetavy", fmt.Sprintf("https://github.com/mpetavy/%s", common.Title()), common.APACHE, false, nil, nil, run, 0)
...
func run() error {
	...
	if flag.NArg() == 0 {
		fmt.Fprintf(os.Stdout, "Please provide a custom command to invoke. Sample: invoke -n 3 cmd /c echo Hello world")
		common.Exit(0)
	}
